...
(async () => {
  ...
  // 1. Create a user
  const uniqueMozart = `mozart_${Date.now()}`;
  const testUser = await db.createUser(uniqueMozart, `${uniqueMozart}@classical.com`, auth.hashPassword('pass123'), 'Wolfgang Amadeus', 0, '🎼');
  ...
  const userRecord = await db.getUserByUsername(uniqueMozart);
  ...
  const completion = await db.recordLessonProgress(testUser.id, 0, 0, 100, 3, 25);
  ...
  const guestUsername = `guest_test_${Date.now()}`;
  const guest = await db.createUser(guestUsername, null, null, 'Maestro Guest', 1, '🎧');
  console.log('Created guest:', guest.username, 'is_guest:', guest.is_guest);
  const claimedUsername = `claimed_${Date.now()}`;
  const claimed = await db.claimGuestAccount(guest.id, claimedUsername, auth.hashPassword('newpass'), 'Now Registered');
  console.log('Claimed account:', claimed.username, 'is_guest:', claimed.is_guest);
  if (claimed.is_guest !== 0 || claimed.username !== claimedUsername) {
    throw new Error('Guest claim failed');
  }

  // 6. Test CMS methods
  const dbUnits = await db.getAllUnits();
  console.log('CMS units fetched:', dbUnits.length);
  if (dbUnits.length === 0) throw new Error('CMS units failed to load');
